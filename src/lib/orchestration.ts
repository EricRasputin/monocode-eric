...
export class Orchestrator {
  ...
  resumeBlocker(leadId: string): Session | undefined {
    ...
    const cwd = run?.status === "paused" ? run.cwd : sessionWorkCwd(lead);
    return this.host
      ?.sessions()
      .find(
        (session) =>
          session.id !== leadId &&
          session.busy &&
          sameCheckout(sessionWorkCwd(session), cwd),
      );
  }
  ...
  async startApproved(
    ...
    const lead = this.host?.session(leadId);
    if (!lead || !sameCheckout(sessionWorkCwd(lead), proposal.cwd))
    ...
    const tasks: OrchestrationTask[] = await Promise.all(
      planned.map(async (task) => ({
        ...
        scopes: await this.store.scopes(sessionWorkCwd(lead), task.files),
  ...
  private async startRun(
    ...
    if (
      previous?.status === "paused" &&
      !sameCheckout(previous.cwd, sessionWorkCwd(lead))
    )
      throw new Error(
    ...
    const [canonicalRoot] = await this.store.scopes(sessionWorkCwd(lead), [
      ".",
    ]);
    const cli = await this.store.enable(leadId, sessionWorkCwd(lead));
    try {
      await this.host!.stop(leadId); // Refresh the child environment before its next turn.
      await this.commit({
        version: 1,
        leadId,
        cwd: sessionWorkCwd(lead),
        canonicalRoot,
        cli,
        status: "active",
  ...
  submissionError(id: string, managed = false): string | null {
    ...
    if (
      own?.status === "active" &&
      !sameCheckout(own.cwd, sessionWorkCwd(session))
    )
