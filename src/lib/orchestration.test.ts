...
describe("local orchestration", () => {
  it("reserves the lead worktree and leaves other checkouts available", async () => {
    const f = setup();
    ...
    await f.start();
    expect(f.manager.run("lead")?.cwd).toBe("/owned/lead");
    expect(f.store.enable).toHaveBeenCalledWith("lead", "/owned/lead");
    expect(f.store.scopes).toHaveBeenCalledWith("/owned/lead", ["."]);
    await f.delegate(["src"]);
    await vi.waitFor(() => expect(f.tasks()[0].status).toBe("running"));
    await vi.waitFor(() =>
      expect(f.host.createWorker).toHaveBeenCalledWith(
        expect.objectContaining({ cwd: "/owned/lead" }),
        expect.anything(),
      ),
    );
    expect(f.manager.submissionError(f.sessions[1].id)).toBeNull();
  });

  it("blocks orchestration when another session uses the same worktree", async () => {
    const f = setup();
    f.lead.worktreeCwd = "/owned/lead";
    f.sessions.push({
      ...newSession("claude", "/repo"),
  ...
  it("starts an approved proposal against the lead's worktree", async () => {
    const f = setup();
    f.lead.busy = false;
    f.lead.worktreeCwd = "/owned/lead";
    await f.manager.startApproved("lead", "card", {
    ...
    expect(f.manager.run("lead")?.cwd).toBe("/owned/lead");
    expect(f.store.scopes).toHaveBeenCalledWith("/owned/lead", ["src/ui"]);
    expect(f.store.enable).toHaveBeenCalledWith("lead", "/owned/lead");
  });
