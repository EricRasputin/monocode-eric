...
fn git_github_pr_diff_for(
    ...
    let (patch, truncated) = if full_context {
        ...
        git_diff_full_context(root, &base, &head, Some(&repo))?
    } else {
        (
...
fn git_diff_full_context(
    root: &Path,
    base: &str,
    head: &str,
    repo: Option<&str>,
) -> Result<(String, bool), String> {
    ensure_git_commit(root, base, repo)?;
    ensure_git_commit(root, head, repo)?;
    ensure_merge_base(root, base, head, repo)?;
...
fn ensure_merge_base(
    ...
    if let Some(remote) = github_fetch_remote(root, repo) {
...
fn github_fetch_remote(root: &Path, repo: Option<&str>) -> Option<String> {
    if let Some(repo) = repo {
        let url = format!("https://github.com/{repo}.git");
        return Some(remote_matching_github_url(root, &url).unwrap_or(url));
    }
...
mod tests {
    ...
    fn git_diff_full_context_includes_distant_lines() {
        ...
        let head = git_run(&dir.0, &["rev-parse", "HEAD"])
            ...
            .trim()
            .to_string();
        let (patch, truncated) = git_diff_full_context(&dir.0, &base, &head, None).unwrap();
        assert!(!truncated);
        assert!(
    ...
    fn git_diff_full_context_errors_without_a_merge_base() {
        ...
        let err = git_diff_full_context(&dir.0, &base, &head, None).unwrap_err();
        assert!(
    ...
    #[test]
    fn github_full_context_fetch_targets_the_selected_inbox_repository() {
        let local = tmp("git-full-context-selected-repo");
        assert!(init_git_commit(&local.0, &[("local.txt", "local\n")]));
        assert!(git(
            &local.0,
            &[
                "remote",
        ...
        assert_eq!(
            github_fetch_remote(&local.0, Some("acme/widget")).as_deref(),
            Some("upstream")
        );
        assert_eq!(
            github_fetch_remote(&local.0, Some("me/widget")).as_deref(),
            Some("origin")
        );
        assert_eq!(
            github_fetch_remote(&local.0, Some("other/widget")).as_deref(),
    ...
    fn ensure_git_commit_fetches_from_gh_resolved_remote() {
        ...
        assert_eq!(
            github_fetch_remote(&local.0, None).as_deref(),
            Some("upstream")
        );
        let spec = format!("{oid}^{{commit}}");
        assert!(git_output(&local.0, &["cat-file", "-e", &spec]).is_none());
        ensure_git_commit(&local.0, &oid, None).unwrap();
